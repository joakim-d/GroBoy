...
int load_bmp(char *bmp_path){
	...
	for(int rows = 0; rows < 16; rows++){
		for(int cols = 0; cols < 16; cols++){
			...
			for(int p_cols = 0; p_cols < cell_w; p_cols++){
				for(int p_rows = 0; p_rows < cell_h; p_rows++){
			...
			for(int p_cols = cell_w -1; p_cols >= 0; p_cols--){
				for(int p_rows = 0; p_rows < cell_h; p_rows++){
	...
	if(bitmap_font != NULL){
		for(int show = 0; text[show] != '\0'; show++){
			if(text[show] == ' '){
				...
				pos_rect.x = pos_x;
				pos_rect.y = pos_y;
				SDL_BlitSurface(bitmap_font, &chars[text[show]], surface, &pos_rect);
				pos_x += chars[text[show]].w + 1;
			}
		}
	}
}
