...
\end{lstlisting}
On retrouve bien tous les registres ainsi que leur taille respective.
\subsection{Algorithme principal}
Une fois la structure définie nous avons mis en place un algorithme servant de "base" au comportement de notre CPU. Pour cela nous nous sommes largement inspiré de \cite{komkom}. Voici son allure générale:
 \begin{lstlisting}
void run(){
	BYTE op_code;
	for(;;)
	{
		...
		update_other_component(cycles);
	}
}
 \end{lstlisting}		

\subsection{Développement des opcodes}
\section{GPU}
\section{MMU}
\section{APU}
\section{Interruptions/Synchronisations}
