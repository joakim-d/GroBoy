...
{
  name={ROMS},
  description={est utilisé pour désigner des copies de puce sur un fichier. On utilisera par exemple "la rom de Mario" pour indiquer la copie de la cartouche du jeu sur un fichier informatique lisible par un émulateur.} 
}
\newglossaryentry{ROM}
{
  name={ROM},
  description={signifie mémoire en lecture seule, elle correspond à la mémoire "morte" car elle ne s'éfface pas lors de la mise hors tension du système.}
}
\newglossaryentry{RAM}
{
  name={RAM},
  description={il s'agie de la mémoire vive, c'est-à-dire qu'elle peut être utilisée par l'ordinateur à n'importe quel moment et rapidement pour travailler avec des applications en temps réel.}
}
