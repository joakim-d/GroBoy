...
Le GPU de la GameBoy est intégré dans le même circuit physique que le CPU. Sa fonction est de générer une image à partir des données contenues dans la VRAM (vidéo ram) puis de l'envoyer à l'écran LCD, il possède différents registres de configuration.
...
Ces périodes de VBlank ou de HBlank ont un nombre de cycles défini, et c'est
durant ces périodes que l'accés aux différentes matrices en écriture va être
...
Trois types de couches peuvent être affichées: la couche background, la couche
...
\item \textbf{La couche Background:}
	Cette couche comprends tous les éléments qui appartiennent au décor du
jeu. Elle est représentée par deux matrices de 256*256 pixels, soit 32*32
tuiles (carrés de 8*8 pixels). La Gameboy peut donc passer d'une matrice à
l'autre pour afficher les différents éléments du décor. Ces matrices étant
d'une taille supérieure à celle de l'écran lors de l'affichage, on gère la position de
...
\item \textbf{La couche Sprites:}
	Cette couche est utilisée pour représenter les objets d'intérêts d'un
...
La couche Sprite peut utiliser deux palettes qui diffèrent en un point de la
...
Enfin toutes les intructions du processeur ont pus être développées principalement grâce aux trois références suivantes \cite{pastraiser}, \cite{clrhome}, \cite{pinaud}.
\section{GPU}
\section{MMU}
...
\subsection{Cheminement}
	...
	Cette librairie s'appelle "blip_buff", et permet de rééchantillonner des ondes audio à partir d'une fréquence d'horloge vers une fréquence d'échantillonage, soit créer une onde discrétisée (donc utilisable par des librairies pour produire du son) à partir d'informations sur une onde carrée. 
