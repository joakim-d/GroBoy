...
Ces deux derniers buffers ont une allocation mémoire dynamique, en effet comme expliqué dans la partie Hardware-Cartouche de jeu selon les jeux leurs tailles sont différentes, il est néanmoins possible de retrouver la taille en mémoire de la ROM et de la RAM de la cartouche grâce aux fichiers de jeux.
...
\end{lstlisting}
On peut voir dans cette fonction que pour chaque plage d'adresse demandée on retourne en fonction soit les valeurs de "cartridge\_rom\_buffer", de "cartridge\_ram\_buffer" ou de "internal\_ram".
